	    ...
	    <script type="text/javascript">
		    ...
		    function drawVisualization() {
		    	var data = google.visualization.arrayToDataTable([
		            ['Date', '영화', '시리즈', '배우', '평균'],
		            <% 
		                List<LogChartVO> logChartData = (List<LogChartVO>) request.getAttribute("reviewCnt");
		                for (LogChartVO entry : logChartData) {
		                    // 각 항목의 카운트와 평균 계산
		                    int movie = entry.getMovie();
		                    int series = entry.getSeries();
		                    int actor = entry.getActor();
		                    double average = (movie + series + actor) / 3.0;
		            %>
		                ['<%= entry.getReportDate() %>', <%= movie %>, <%= series %>, <%= actor %>, <%= average %>],
		            <% 
		                } 
		            %>
		        ]);
                ...
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Dashboard > Review Card</li>
                        </ol>
                        <div class="row">
